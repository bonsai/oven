# ADR 0001: Bun + Hono + FFI API skeleton for stable-diffusion.cpp
...
We need a lightweight API server that exposes Stable Diffusion generation over HTTP without Python dependencies. The target runtime is Bun with TypeScript, and the inference backend is the `stable-diffusion.cpp` shared library (DLL/.so/.dylib). The project should also be operable when the native library is absent so that API contracts can be exercised during early development.

## Decision

- Use **Bun** as the runtime for its fast startup and native TypeScript support.
...
- **Node + Express**: rejected due to heavier runtime and slower cold start compared to Bun.
